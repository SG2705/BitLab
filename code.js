...
class Gate {
  ...
  connectTo(targetNode, connectFromHandle = null, connectToHandle = null) {
  ...
  disconnectFrom(targetNode, connectFromHandle = null, connectToHandle = null) {
    if (this._circuit === null) {
      throw new Error(`${this.gid} is not attached to circuit`);
    }
    if (targetNode.circuit !== this._circuit) {
...
class Connection {
  ...
  saveConnection(
    sourceNodeId,
    targetNodeId,
    connectFromHandle = null,
    connectToHandle = null,
  ) {
...
class Circuit extends Gate {
  ...
  addProbe(probeId, targetId, index = null) {
  ...
  addScope(scopeId, nodeId, index = null) {
  ...
  static importCircuit(config) {
    ...
    config.nodes.forEach((nodeConfig) => {
      if (nodeConfig.isCircuit) {
        newCircuit.addNode(this.importCircuit(nodeConfig));
      } else {
        newCircuit.addNode(Gate.import(nodeConfig));
      }
    });
